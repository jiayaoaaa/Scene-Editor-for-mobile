define(["draggable"], function(avalon) {
    ...
    var resizable = avalon.bindingHandlers.resizable = function(data, vmodels) {
       ...
       //修改target绑定为document，解决鼠标离开元素时不能拖动
        ...
        var _drag = model.resizable.drag || avalon.noop;
        var _start = model.resizable.start || avalon.noop;
        var _stop= model.resizable.stop; 
        ...
        options.beforeStart = function(event, data) {
            ...
            _start.call(target[0], event, data); //触发用户回调
        ...
        options.beforeStop = function(event, data) {
            if (data.dir) {
                ...
                event.type = "resizeend";
                 _stop.call(target[0], event, data); //触发用户回调
