 ...
 */
define(["draggable", "text!"+SP_URL_STO+"vendor/avalon/colorpicker/avalon.colorpicker.tpl", "css!vendor/avalon/colorpicker/avalon.colorpicker.css"], function(avalon, sourceHTML){
	...
	var widget = avalon.ui.colorpicker = function(element, data, vmodels){
		...
		var vm = {
			...
			$init: function(){
				...
				if(!isHexColorValid(element.value)){
					avalon.log(element.value + ' colorpick不是合法的十六进制颜色表示法');
		...
		function renderView(){
			...
			if(vmodel.autoHide){
				...
				avalon.bind(cp_btn, "click", function(){
							vmodel.toggle = false;
				                    var val=element.value;
								  element.value=val
				})
